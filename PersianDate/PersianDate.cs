...
public struct PersianDate : IComparable<PersianDate>
{
    ...
    public static void yearMonthDay(uint x, out int y, out int m, out int d)
    {
        var a = (int)(x / period33y);
        y = a * 33;
        a = (int)(x % period33y);
        int b;
        if (a < p33p1)
        ...
        {
            var rem = (a - p33p1 - p33p2 - p33p3) % period4y;
    ...
    public static readonly PersianDate MaxValue = new(maxYear, maxMonth, maxDay);


    //min date: "1/1/1"
    private const int minYear = 1, minMonth = 1, minDay = 1;
    public static readonly PersianDate MinValue = new(minYear, minMonth, minDay);

    //3/21/0622 12:00:00 AM
    private static readonly DateTime firstDateDateTime = new(622, 3, 21);
    ...
    public static uint days(int y, int m, int d)
    {
        uint r;
    ...
    public static PersianDate Today => new PersianDate(DateTime.Today);
    ...
    /// <param name="result">
    ///     If the conversion succeeds, this parameter will contain the PersianDate value
    ///     equivalent to the persian date specified in the first parameter, otherwise it will have the value of date 1/1/1
    /// </param>
    ...
    private readonly uint n; //the only field, stores the number of days passed 1/1/1

    public PersianDate(uint n)
    {
        this.n = n;
    }
    ...
    public PersianDate(int year, int month, int day)
    {
        ...
        if (comp(year, month, day, minYear, minMonth, minDay) < 0)
            throw new ArgumentOutOfRangeException(
                string.Format("The date ({0}/{1}/{2}) is less than the minimun acceptable date", year, month, day));
        if (comp(year, month, day, maxYear, maxMonth, maxDay) > 0)
            throw new ArgumentOutOfRangeException(
                string.Format("The date ({0}/{1}/{2}) is greater than the maximum acceptable date", year, month, day));
        n = days(year, month, day);
    }

    /// <summary>
    ///     Initializes a new instance of the PersianDate structure set to the persian date equivalent to the date specified in
    ...
    /// </summary>
    public PersianMonth MonthAsPersianMonth => (PersianMonth)Month;

    /// <summary>
    ///     Gets the day of the week represented by this PersianDate instance.
    /// </summary>
    public DayOfWeek DayOfWeek => (DayOfWeek)((n + 4) % 7);

    /// <summary>
    ///     Gets the day of the week as PersianDayOfWeek represented by this PersianDate instance.
    /// </summary>
    public PersianDayOfWeek PersianDayOfWeek => (PersianDayOfWeek)((n + 4) % 7);
